...
DECLARE
    v_trimester TEXT;
BEGIN
    SELECT trimester::text INTO v_trimester
    FROM lessons
    WHERE class_id = p_class_id AND trimester IS NOT NULL
    LIMIT 1;
    ...
    SELECT gq.q_id, gq.q_text, gq.opt_a, gq.opt_b, gq.opt_c, gq.opt_d, gq.resp, gq.lesson_number
    FROM guaranteed gq
    ORDER BY random();
END;
$function$;