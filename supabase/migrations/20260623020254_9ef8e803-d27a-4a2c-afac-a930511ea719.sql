...
BEGIN
    ...
    ranked AS (
        SELECT
            q_id, q_text, opt_a, opt_b, opt_c, opt_d, resp, l_num,
            ROW_NUMBER() OVER (PARTITION BY l_num ORDER BY random()) AS rn
        FROM lesson_questions
    ),
    guaranteed AS (
        SELECT q_id, q_text, opt_a, opt_b, opt_c, opt_d, resp, l_num
        FROM ranked
        WHERE rn <= 2
    )
    SELECT gq.q_id, gq.q_text, gq.opt_a, gq.opt_b, gq.opt_c, gq.opt_d, gq.resp, gq.l_num
    FROM guaranteed gq
    ORDER BY random();
END;
$function$;