...
WITH
-- Resolve "person_key": usa user_id quando disponível, senão participant_id
...
-- Escolhe um registro "canônico" de participante por person_key (o mais antigo)
...
-- Mapeia TODOS os participant_ids de cada pessoa
person_participants AS (
  SELECT person_key, participant_id, user_id
  FROM participant_person
),
-- Melhor tentativa por (pessoa, lição/quiz) — não por participant
...
-- Leitura: agrupa por user_id (não duplica entre participants do mesmo user)
...
-- Recria a view de igrejas (média = sum / count distinct de alunos participantes)
