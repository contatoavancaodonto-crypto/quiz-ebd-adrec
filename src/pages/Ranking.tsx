      ...
      <PageShell contentClassName="px-4 py-4 max-w-lg mx-auto w-full space-y-4">
        ...
        {(mode === "classic" || isInter) && enabledForMode && ranking.length > 0 && !!activeSeason && (
          <div className="mb-3 px-3 py-2 rounded-xl bg-primary/10 border border-primary/30 text-[11px] text-foreground/90 flex items-center gap-2">
            ...
            <span>
              <strong className="font-semibold">Ranking parcial</strong> · {isInter ? "nota média da igreja" : "pontuação acumulada"} do {trimester}º trimestre <span className="text-muted-foreground">(total possível: 100 pts · 65 lições + 13 leitura + 22 provão)</span>
            </span>
          </div>
        )}
